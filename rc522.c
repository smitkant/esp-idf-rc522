...
{
    uint8_t ver = rc522_read(rc522, 0x37);
    ESP_LOGI(TAG,"version is - 0x%02x",ver);
    return ver;
}
...
esp_err_t rc522_start(rc522_handle_t rc522)
{
    ...
    if(! rc522->initialized) {
        ...
        for(uint8_t i = test_val; i < test_val + 2; i++) {
            if((err = rc522_write(rc522, test_addr, i)) != ESP_OK || (val=rc522_read(rc522, test_addr)) != i) {
                ESP_LOGE(TAG, "Read/write test failed - %d ,0x%02x",err,val);
                rc522_destroy(rc522);
                return err;
            }
        }
...
static esp_err_t rc522_spi_send(rc522_handle_t rc522, uint8_t* buffer, uint8_t length)
{
    ...
    t.addr = (buffer[0] << 1) & 0x7E;
    t.length = 8 * 1;
    t.tx_buffer = buffer+1;

    return spi_device_polling_transmit(rc522->spi_handle, &t);
}
...
static esp_err_t rc522_spi_receive(rc522_handle_t rc522, uint8_t* buffer, uint8_t length, uint8_t addr)
{
    ...
    // } else { // Fullduplex
        ...
        // assert(ret == ESP_OK);
        if(ESP_OK != (ret = spi_device_polling_transmit(rc522->spi_handle, &t))) {
...
static void rc522_task(void* arg)
{
    ...
    while(rc522->running) {
        ...
        if(! serial_no_array) {
            ...
            ESP_LOGI(TAG,"rcvd tag %lld",rc522_sn_to_u64(serial_no_array));
            rc522_tag_t tag = {
